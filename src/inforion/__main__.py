...
def extract(program, outputfile):
    ...
    if not "outputfile" in locals() or not outputfile:
        logging.info("\033[91m" + "Error: Output filename is missing" +
                     "\033[0m")
...
@click.option("--mergesheet2",
              "-n",
              help="Please define the second merge file")
@click.option("--mergeoutput",
              "-o",
              help="Please define the output file for merge")
@click.option("--mergecol",
              "-c",
              help="Please define the column criteria for merge")
...
@click.option("--service_name",
              "-s",
              help="Service name. e.g. BusinessPartner, SalesOrder")
